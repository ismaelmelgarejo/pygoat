pipeline {
  ...
  environment {
    ...
    // - En Docker Desktop (Win/Mac) host.docker.internal funciona.
    // - En Linux, si no funciona, usa: http://172.17.0.1:<puerto>
    ...
    stage('Secrets Scan - gitleaks') {
      steps {
        sh """
          ...
          # (Opcional) si quieres fallar por secretos:
          if [ "\$LEAKS" -gt 0 ]; then
    ...
    stage('SAST - Bandit') {
      steps {
        sh """
          ...
          jq --argjson bh "\$BANDIT_HIGH" '. + {bandit:{high:$bh}}' ${REPORTS_DIR}/summary.json > ${REPORTS_DIR}/summary.tmp.json
    ...
    stage('SCA - SBOM -> Dependency-Track -> FPF Export') {
      ...
      steps {
        sh """
          ...
          #    Devuelve true mientras procesa, false cuando termina.
          ...
          # 4) Obtener UUID del proyecto (lookup) y exportar findings en FPF
          #    Nota: la REST API está documentada vía OpenAPI en /api/openapi.json (backend). :contentReference[oaicite:15]{index=15}
          #    URL-encode con python.
          ...
          PROJ=\$(curl -sS -H "X-Api-Key: ${DTRACK_API_KEY}" "${DTRACK_BACKEND_URL}/api/v1/project/lookup?name=\$ENCODED_NAME&version=\$ENCODED_VER")
          ...
          # 5) Security gate (CRITICAL/HIGH) usando el FPF export (vulnerability.severity)
          ...
          jq --argjson dc "\$DT_CRIT" --argjson dh "\$DT_HIGH" '. + {dependencytrack:{critical:$dc, high:$dh}}' ${REPORTS_DIR}/summary.json > ${REPORTS_DIR}/summary.tmp.json
          mv ${REPORTS_DIR}/summary.tmp.json ${REPORTS_DIR}/summary.json
        """
      }
    }
