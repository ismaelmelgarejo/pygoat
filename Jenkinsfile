      ...
      steps {
        sh '''#!/usr/bin/env bash
          ...
          for i in $(seq 1 60); do
            PROCESSING="$(curl -sS -H "X-Api-Key: $DTRACK_API_KEY" \
              "$DTRACK_BACKEND_URL/api/v1/bom/token/$TOKEN")"
            echo "processing=$PROCESSING (try $i/60)"
            [ "$PROCESSING" = "false" ] && break
            sleep 5
          done

          # 4) Lookup proyecto (sin problemas de encoding)
          PROJ="$(curl -sS --get \
            -H "X-Api-Key: $DTRACK_API_KEY" \
            --data-urlencode "name=$PRODUCT_NAME" \
            --data-urlencode "version=$BUILD_NUMBER" \
            "$DTRACK_BACKEND_URL/api/v1/project/lookup")"
    ...
    stage('Security Gates') {
      steps {
        sh '''#!/usr/bin/env bash
          ...
          echo "Gate => Bandit HIGH=$BANDIT_HIGH | DTrack CRIT=$DT_CRIT HIGH=$DT_HIGH"
